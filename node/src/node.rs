...
use crate::{config::NodeConfig, cover::CoverTrafficGenerator,
    metrics::NodeMetrics, mixer::PoissonMixer, router::PacketRouter};
use zksn_crypto::sphinx::{SphinxPacket, PACKET_SIZE};

pub struct MixNode { config: NodeConfig, listener: TcpListener }
...
impl MixNode {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        let (tx_out, rx_out) = mpsc::channel::<(String, SphinxPacket)>(self.config.mixing.max_queue_depth);
        ...
        loop {
            match self.listener.accept().await {
                Ok((stream, peer)) => {
                    ...
                    let tx = tx_in.clone();
                    tokio::spawn(async move {
                        if let Err(e) = handle_conn(stream, tx).await { warn!("{peer}: {e}"); }
...
async fn handle_conn(mut s: tokio::net::TcpStream, tx: mpsc::Sender<SphinxPacket>) -> Result<()> {
    ...
    if let Ok(pkt) = bincode::deserialize::<SphinxPacket>(&buf) { tx.send(pkt).await?; }
    Ok(())
}
