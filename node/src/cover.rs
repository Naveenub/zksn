...
pub struct CoverTrafficGenerator { config: MixingConfig, tx: mpsc::Sender<SphinxPacket> }

impl CoverTrafficGenerator {
    pub fn new(config: MixingConfig, tx: mpsc::Sender<SphinxPacket>) -> Self { Self { config, tx } }

    pub async fn run(&mut self) -> Result<()> {
        if self.config.cover_traffic_rate == 0 { return Ok(()); }
        ...
        loop {
            ...
            match pkt {
                Ok(p) => { trace!("Cover {}", if use_loop { "LOOP" } else { "DROP" });
    ...
    fn build_route(&self) -> Vec<NodeIdentity> {
        (0..3).map(|_| { let mut k = [0u8; 32]; rand::thread_rng().fill_bytes(&mut k);
            NodeIdentity { public_key: k } }).collect()
    }
}
...
mod tests {
    use super::*;
    #[tokio::test] async fn test_emits_packets() {
        let cfg = MixingConfig { cover_traffic_rate: 100, poisson_lambda_ms: 200,
            max_queue_depth: 64, loop_cover_fraction: 0.5 };
    ...
    #[tokio::test] async fn test_disabled_at_zero() {
        let cfg = MixingConfig { cover_traffic_rate: 0, poisson_lambda_ms: 200,
            max_queue_depth: 64, loop_cover_fraction: 0.5 };
        let (tx, mut rx) = mpsc::channel::<SphinxPacket>(64);
        let mut gen = CoverTrafficGenerator::new(cfg, tx);
