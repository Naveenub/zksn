...
contract ZKSNGovernance {
    event ProposalCreated(bytes32 indexed proposalId, bytes32 contentHash, uint256 votingEndsAt, address indexed proposer);
    ...
    function castVote(
        ...
        if (voteYes) { p.yesVotes++; } else { p.noVotes++; }
    ...
    function getProposal(bytes32 proposalId) external view returns (
        bytes32 contentHash, uint256 votingEndsAt, uint256 yesVotes,
        uint256 noVotes, bool executed, bool passed
    ) {
        Proposal storage p = proposals[proposalId];
