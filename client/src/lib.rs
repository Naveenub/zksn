...
impl ZksnClient {
    ...
    pub async fn send(&self, recipient_pubkey_hex: &str, payload: &[u8]) -> Result<()> {
        ...
        if bytes.len() != 32 {
            return Err(anyhow::anyhow!("Recipient pubkey must be 32 bytes hex (64 chars)"));
        }
...
mod tests {
    ...
    async fn test_routing_pubkey_is_deterministic() {
        ...
        let cfg1 = ClientConfig { key_path: Some(key_path.clone()), ..Default::default() };
        let cfg2 = ClientConfig { key_path: Some(key_path), ..Default::default() };
    ...
    async fn test_send_fails_without_peers() {
        let config = ClientConfig { bootstrap_peers: vec![], ..Default::default() };
