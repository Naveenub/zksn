...
struct Cli {
    ...
    #[arg(short, long, global = true)] key: Option<String>,
    #[arg(short, long, global = true, default_value = "[::1]:9001")] node: String,
    #[arg(short, long, global = true)] debug: bool,
}

#[derive(Subcommand)]
enum Commands {
    Identity { #[command(subcommand)] action: IdentityCmd },
    Send { recipient: String, message: String },
    Receive,
    Wallet { #[command(subcommand)] action: WalletCmd },
}

#[derive(Subcommand)]
enum IdentityCmd {
    Generate { #[arg(short, long)] output: Option<String> },
    Show,
}
...
async fn main() -> Result<()> {
    ...
    let config = ClientConfig { key_path: cli.key.clone(), entry_node: cli.node.clone(),
    ...
    match cli.command {
        ...
        Commands::Receive => {
            let client = ZksnClient::new(config).await?;
            println!("{} {}", "Listening as:".dimmed(), client.fingerprint().cyan());
            let mut rx = client.receive().await?;
            while let Some(payload) = rx.recv().await {
                println!("{} {}", "▶".green().bold(), String::from_utf8_lossy(&payload));
            }
            Ok(())
        }
        Commands::Wallet { action } => {
            match action {
                WalletCmd::Balance => println!("{}", "Balance: (see economic/src/cashu.rs)".yellow()),
                WalletCmd::Topup { amount } => println!("Topup {amount}: (see economic/src/cashu.rs)"),
            }
            Ok(())
        }
    }
...
async fn handle_identity(action: IdentityCmd, config: &ClientConfig) -> Result<()> {
    ...
    match action {
        IdentityCmd::Generate { output } => {
            ...
            println!("{} {}", "Fingerprint:".dimmed(), pub_.fingerprint().yellow());
            println!("{} {}", "Public key: ".dimmed(), hex::encode(pub_.as_bytes()).green());
